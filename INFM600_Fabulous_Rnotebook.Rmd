...
For the first graph, "Residuals vs Fitted", this graph shows the relationship between estimated reponses on x axis and residuals on the y axis, which let us detect non-linearity, unequal error variances and outliers, which are the assumptions of multiple linear regression. From this graph, since the residuals looks sort of "bounce radomly " around the 0 line, the assumption that the relationship is lineaer is reasonable. Meanwhile, because most residuals form a horizonal band around the 0 line, it suggests taht the variances of the error are equal. However, there are some residuals that stand out from the 0 line, suggesting there are outliers.
...
For the forth graph, "Residuals vs Leverage", this graph shows the standardized residuals against levelrage. We can found most residuals are symmetrically centered around zero and reach 3-4 standard deviations aways from zero, the same as the expectation of the normal distribution. Meanwhile, although there is an extremly large outlier labed "1621", the cook's distance is also very small and no points have a large Cook's distance that larger than 0.5.

Therefore, from these four plots, we check the assumptions of multiple lineaer regression and most of assumptions are satisfied. Although there are some outliers, they do not have a large effect because of our large sample size. 
...
p1 <- ggplot(p) +
  ...
  geom_text_repel(data=s, aes(x=long, y=lat, label=s$name), size=4) +
  theme_light(base_size=10) +
  scale_colour_gradientn(colors=c("#dddddd", "#20beff"), limits=c(0, max(p$Ridership)), name="Number of Trips") +
...
ggsave("DC_trips.pdf", p1, width=10, height=8, units="in")
